...
const schema = z.object({
  ...
  email: z.string().email("A real email so we can send you the code"),
});
...
export default function ReferPage() {
  ...
  async function onSubmit(values: FormValues) {
    setError(null);
    try {
      const { referral } = await claimReferral(values.email, values.name);
      setReferral(referral);
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
          : "Couldn't claim a code. Try again.",
      );
    }
  }
          ...
          <div className="grid lg:grid-cols-[1.1fr_1fr] gap-10 items-start">
            <motion.div {...fadeUp}>
              ...
              <h1 className="mb-4 text-[clamp(2rem,3.6vw+0.5rem,3.25rem)]! leading-[1.05]!">
                Earn <span className="text-gold">$25</span> for every friend who
                enrolls.
              </h1>
              <p className="text-[16.5px] text-muted-foreground leading-relaxed mb-6 max-w-xl">
                ...
                {COURSE_META.productTitle}, they save $25 and you earn $25 cash —
                paid out after the 30-day refund window.
              </p>
              ...
              {!referral ? (
                ...
                >
                  ...
                  <p className="text-[11.5px] text-muted-foreground text-center mt-4">
                    Returning? Submit the same email — we'll give you the same
                    code.
                  </p>
                </form>
              ) : (
                <ReferralDashboard referral={referral} copied={copied} setCopied={setCopied} />
              )}
            </motion.div>
          </div>
        </div>
        ...
        <div className="max-w-[1000px] mx-auto px-6 md:px-10 py-16">
          <motion.div {...fadeUp} className="max-w-2xl mb-10">
            ...
            <h2 className="text-[26px]! md:text-[28px]! mb-3">
              Three steps. No tracking pixels.
            </h2>
          </motion.div>
        ...
        <div className="max-w-[860px] mx-auto px-6 md:px-10 py-14">
          <motion.div {...fadeUp}>
            ...
            <ul className="space-y-2 text-[14px] text-muted-foreground leading-relaxed">
              <li>
                Conversions are marked <strong className="text-navy">pending</strong>{" "}
                until the 30-day refund window passes. After that they move to{" "}
                <strong className="text-navy">confirmed</strong> and become eligible
                for payout.
              </li>
              <li>
                Payouts go to the email on file via Stripe or PayPal. Anything
                under $25 rolls forward to the next month.
              </li>
              <li>
                Self-referrals (using your own code on your own enrollment) are
                detected and excluded.
              </li>
              <li>
                We don't share your email with anyone you refer — only your
                first name appears on the landing page.
              </li>
            </ul>
          </motion.div>
        </div>
    ...
    <div className="card-base p-6 md:p-7 bg-white shadow-[0_12px_40px_rgba(11,26,58,0.08)] space-y-5">
      <div>
        ...
        <h3 className="text-[22px]! mb-4">
          Send this to other business owners.
        </h3>
      </div>
      ...
      <p className="text-[11.5px] text-muted-foreground text-center">
        Confirmed earnings move to <span className="text-success font-semibold">payout-eligible</span>{" "}
        31 days after each enrollment.
      </p>
    </div>
  );
}
...
function Stat({
  ...
  return (
    <div className="card-base p-4 bg-white">
      ...
      <div className="text-[22px] font-bold text-navy tabular-nums mt-1">
        {value}
      </div>
    </div>
  );
